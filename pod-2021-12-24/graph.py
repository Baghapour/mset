...
def linear(fname='Test',x=None,f=None,ndict=None,pallet=lineBlack,log=(0,0),save=True,show=False,ar=False):
...
def compare(fname='Compare',x=None,f=None,log=(0,0),ndict=None,legend=None,pallet=None,save=True,show=False):
    if f is None:     
...
def take(csv=None,f=None):
    ...
    X    = data[:,head.index(cmn.csvDict['X'])]
    Y    = data[:,head.index(cmn.csvDict['Y'])]
    if f is None:
        return X,Y
    else:
        F = data[:,head.index(cmn.csvDict[f])]
        return X,Y,F



def interpolate(grid=None,field=None,line=None):
    X = grid[0]
    Y = grid[1]
    F = field
    Z = interp2d(X,Y,F)
