...
class pod(object):
    '''
       class pod (proper-orthognal decomposition)
       ------------------------------------------
       * POD analysis on CSV data
       * field: the original info (data,indexf)
       * dec: the decomposed info (U,S,VT,indexd)
       * nsamp: number of samples
       ------------------------------------------
       * init     : set data folder and nsamp
       * define   : define field (original) data
       * domain   : managing field data
    ...
    def normalize(self,f=None,method=None):
        if f is None or isinstance(f,str) is False:
            sys.exit('pod-normalize: f is not set properly.')
        if method is None:
            sys.exit('pod-normalize: method is not set.')
        try:    idx = self.indexf(f)
        except: sys.exit('pod-normalize: '+f+' is not defined in field.')
        if method == 'min-max':
            dat = self.data[idx]
            ...
            self.min.append(min)
            self.max.append(max)
            print('pod-normalize: field '+f+'is normized with method: '+method)
        else:
